...
このリポジトリは 11 の Skill (`prep-extractor` / `prep-architect` / `prep-builder` / `prep-deployer` / `prep-output-comparator` / `prep-pds-augmenter` / `prep-schedule-designer` / `prep-workbook-repointer` / `prep-pulse-repointer` / `prep-pds-backfiller` / `prep-migration-planner`) で構成されています。各 Skill の役割と副作用区分は [CLAUDE.md](CLAUDE.md#skill-構成) にまとめてあります。
