...
2. **Tableau MCP を設定する** — parity 比較 (`prep-output-comparator`) などが Tableau MCP のツールを使います。`.codex/config.toml` の `[mcp_servers.tableau]` テンプレート (既定は全行コメントアウト) を、自分の環境の Tableau MCP 実装に合わせて有効化するか、ユーザーの Codex config 側で設定します。本リポジトリは特定の MCP 実装・起動コマンドを規定しません
