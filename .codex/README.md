...
2. Tableau MCP を使う場合は `config.toml.template` を `config.toml` にコピーし (`.gitignore` 済のためマシン固有値を書いてよい)、`[mcp_servers.tableau]` テンプレートのコメントを外して自分の環境の Tableau MCP 実装に合わせて埋める。
