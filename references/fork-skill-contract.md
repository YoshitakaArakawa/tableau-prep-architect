---
purpose: context:fork で動く read-only 後工程 Skill (prep-output-comparator / prep-schedule-designer / prep-workbook-repointer) が共通で守る呼び出し契約を 1 箇所に集約する。caller 入力の渡し方・戻り値の Timing・失敗時の停止規律・認証失効の扱いの 4 点
note: 全 fork Skill に共通する骨だけを規定する。各 Skill 固有の入力表・固有の「よくある失敗」パターン列挙は各 SKILL.md に残す。書き込み系 (prep-deployer / prep-pds-augmenter / prep-pds-backfiller) は承認・失敗観測を主会話で扱うため fork せず、この契約 (特に §2 Timing) は適用されない — 適用是非は各 SKILL.md が明示する
---
...
スクリプトや MCP 呼び出しが失敗したら **その時点で停止し、caller にエラーを返す**。read-only な後工程 Skill は autonomous-recovery をしない — リトライするか・入力を直すか・人間作業を挟むかの判断は、副作用を持つ caller (メインエージェント) の責務。各 Skill 固有の「よくある失敗」パターンとその差し戻し文言は各 SKILL.md に列挙する。
