---
purpose: context:fork で動く後工程 Skill (prep-output-comparator / prep-schedule-designer / prep-workbook-repointer) が共通で守る呼び出し契約を 1 箇所に集約する。caller 入力の渡し方・戻り値の Timing・失敗時の停止規律・認証失効の扱いの 4 点。多くは read-only だが、prep-workbook-repointer の repoint モード (WB republish) のようにサーバー書込を持つモードにも同じ契約が適用される
