---
purpose: decomposition plan.json のスキーマ仕様 — tableau-prep-architect が emit し、render_plan_md.py が Stop 2 用 md をレンダリングし、tableau-prep-builder の build_from_plan.py が .tfl 群を機械組み立てする単一の設計成果物
...
`server` / `flow_projects` / `ds_projects` / `original.outputs` は [gen_plan_skeleton.py](../.claude/skills/tableau-prep-architect/scripts/gen_plan_skeleton.py) が deploy-context.md / flow.json / input-dispatch-mech.json から機械生成する。architect が手で埋めるのは設計フィールドのみ。
...
```jsonc
{
  ...
  "transforms": [                         // op ∈ {rename, cast, hide}
    { "op": "rename", "column_name": "[<uuid>]", "to_caption": "数量" },
    { "op": "rename", "column_name": "[<uuid>]", "to_caption": "単価 (USD)" },
    { "op": "cast",   "column_name": "[<uuid>]", "to_caption": "金額", "to_datatype": "real" },
...
`column_name` は input-dispatch-mech.json の `fields[].name_bracketed`。**`to_caption` は元の内部名へのピン留めに限る** — 原則は現行 caption そのまま (skeleton 初期値が正解)、例外は actions-split で stg に吸収した正規化 rename (上例の `単価 (USD)`) のみ。英語等への semantic translation は禁止 (下流の verbatim 転写式が元名で列参照するため。正典: [input-policy.md §命名レジーム](input-policy.md))。
...
| `passthrough_pds` | 元フローが読んでいた**既存 PDS** を新規 LSP で読む | `pds_name`, `project_path` (**フルパス**。builder は leaf 化せずそのまま LSP の projectName にする), `luid`, `dbname` (**強く推奨**: input-dispatch の `pds.dbname`。あれば deployer の dbname patch 往復が不要。無いと builder は placeholder + WARNING を出し deployer patch に委ねる), `replaces_steps` |
| `transplant` | 元 Input ノードを**接続ごと verbatim 転写** (passthrough の第一選択。fields / dbname / Input renames を無変更で保持) | `step` |

**`replaces_steps` = このフローに含めた step の、元フローでの直接の親 step** (= その入力 PDS が表す上流終端ノード)。両ケースの例:

- `upstream_pds` (本 plan の上流フローが publish する PDS): 上流フローの**最終 step**。例: 上流 int の最終 Clean が #8 で、このフローが #9 を含むなら `replaces_steps: [8]`
- `passthrough_pds` / `transplant` (元フローが直接読んでいた入力): **元 Input ノード自身の step**。例: 元 LSP Input #1 が Clean #3 を feed していたなら `[1]` (skeleton の `_passthrough_hints` が出す値そのまま)

namespace 継承 (Union/Join の入力識別) と lineage 検証の両方がこの値から導出される。**上流 PDS を 2 系統として直接読む場合** (バッファノード無しの直接 self-read) は inputs entry を 2 つに分ける。元フローが 1 本のエッジから in-flow のバッファノード (空 Clean 等) 経由で二系統に分岐している self-union は、entry 1 本 (`replaces_steps` に親 step 1 つ) で配線される (分岐は含めたノード側が保持するため)。
...
- **式トークン整合**: 転写対象 actions の式が参照する `[列名]` トークンを、上流 PDS の公開スキーマ (augment stg は transforms 適用後、passthrough/transplant は元 Input の fields) ∪ フロー内で生成される名前と照合する。命名レジーム崩れ (semantic translation された stg 等) を「run で落ちる前」に検知する。上流に kind=tfl (build 前でスキーマ静的不明) が混ざるフローは照合 skip (note で明示)
