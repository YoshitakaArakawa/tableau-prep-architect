...
note: tableau-prep-builder が init し、tableau-prep-deployer が publish/run/resolve-luids で更新し、tableau-pds-comparator が読み取る。複数 Skill 共有の形式契約のためリポ直下に置く
---
...
| Skill | 操作 |
|---|---|
| tableau-prep-architect | 直接書かない。plan.json の `flows[].source_original_output_name` で対応関係を表明 |
| tableau-prep-builder | **init**: `.tfl` 群 build 完了後に manifest を新規作成 (`--plan-json` + flows/ 直下のスキャンから組む。build_from_plan.py の `--manifest` で自動実行) |
| tableau-prep-deployer | **update-publish**: 各 .tfl を publish するたびに `decomposed_flows[].publish` を更新 / **update-run**: 各 run 後 `decomposed_flows[].run` を更新 / **resolve-luids**: chain 完了後 Metadata API で全 LUID を解決 |
| tableau-pds-comparator | **read-only**: ペア解決 (`scripts/resolve_pairs.py`) の唯一の入力 |
| tableau-workbook-repointer / tableau-pulse-repointer | **read-only**: design モードの repoint join (後述 §repoint join model) の RIGHT side |
...
| `kind` | string | ✅ | `tfl` (Prep flow を publish + run) または `pds_augment` (tableau-pds-augmenter で Live PDS を直接 publish、run なし) |
...
```
[tableau-prep-builder]            init                      → publish-manifest.json 生成
                            ↓ (全 flow status=pending, luid=null)
[tableau-prep-deployer publish]   update-publish (per .tfl) → publish.status / .flow_luid / .published_at セット
                            ↓ (publish 完了)
[tableau-prep-deployer run]       update-run (per flow)     → run.status / .finish_code / .run_at セット
                            ↓ (全レイヤ完走)
[tableau-prep-deployer]           resolve-luids             → original.flow_luid + 全 PDS LUID を Metadata API で解決
                            ↓
[tableau-pds-comparator]  read-only                 → ペア解決 → 比較
```
...
tableau-workbook-repointer / tableau-pulse-repointer の design モードは、本 manifest を RIGHT side に旧→新 PDS 対応をローカル join で機械確定する。両 Skill の build plan スクリプト共通の正典:
...
`schema_version` の bump 規律は、本 manifest を読み取る Skill / スクリプトが現リポ内 (tableau-prep-builder / tableau-prep-deployer / tableau-pds-comparator) に閉じている間は適用しない。外部から固定的に消費される public contract になった時点から bump 規律に切り替える。
