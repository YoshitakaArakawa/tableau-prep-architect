...
stg レイヤの各 entry の取扱は、prep-extractor の Phase C が生成する **`input-dispatch.md` (status: confirmed)** が source-of-truth。architect は dispatch を読み、Input 単位の取扱を以下の 3 値に振り分けて plan を組む:

| Input dispatch | stg entry を plan に置くか | 後段の挙動 |
|---|---|---|
| `passthrough` (PDS 整形済) | **置かない** | int 等の下流 entry が `Inputs` で **元 PDS を直接参照** (LoadSqlProxy で project_path + name + LUID) |
| `augment` (vconn / 整形が必要な PDS) | 置く + `Materialization: live_pds` | [prep-builder](../.claude/skills/prep-builder/SKILL.md) が .tfl を作らず、`flows/staging/<name>.augmenter.json` (prep-pds-augmenter spec) を emit。[prep-deployer](../.claude/skills/prep-deployer/SKILL.md) は `augment_pds.py` を呼んで publish (run フェーズは skip) |
| `tfl` (将来的に non-vconn な複雑 stg を入れる場合) | 置く + `Materialization: tfl` | 従来通り .tfl を組み立てて publish + run |
| `block` (direct_db / extract / unknown) | session 全体停止 | decompose しない。ユーザーに Cloud 側整備を依頼 |

**判定の責任分担**:

- **input-dispatch.md (Phase C + user confirmation)** が `passthrough | augment | block` を決める
- **architect** は dispatch を読み、`augment` の Input には Materialization 付き stg entry を、`passthrough` の Input には stg entry を置かず int 等の Inputs に直書き、`block` があれば decompose を中断
...
- `Included original steps` と `Upstream lineage` は **省略可** (元 SuperTransform の actions を transcribe しているので element 単位の追跡は不要)
- `Description` は引き続き必須
...
- stg input が vconn 以外で stg に column 操作以上 (row filter / cleansing) が必要なケースは layer-responsibilities.md 改訂で int 送りに分類変更済 — このパターンが残っていたら architect は decompose 設計を見直すべきサイン
- non-vconn な passthrough stg (= 列リネーム / 型キャストだけしたいが vconn ではない) はまだ自動化されていない。当面は `Materialization=tfl` で .tfl を組む
