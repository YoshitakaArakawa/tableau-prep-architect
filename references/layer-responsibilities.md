...
stg は **column-level 操作だけ** に閉じる。row-level 操作 (フィルタ / 値置換 / NULL 補完) や集約・結合は intermediate へ。これは [tableau-pds-augmenter](../.claude/skills/tableau-pds-augmenter/SKILL.md) で stg を Live PDS (vconn-backed) として表現できる範囲に合わせた制約 — augmenter が表現可能なのは rename / cast / hide のみ。
...
**マテリアライゼーション**: stg の Input 単位の取扱は tableau-prep-extractor Phase B の `input-dispatch-mech.json` を architect が読んで決める (正典は [decomposition-plan-format.md §Input dispatch と stg materialization](decomposition-plan-format.md#input-dispatch-と-stg-materialization))。経路:

- **`pds` 解決済 → passthrough**: stg .tfl を作らず、下流 int が元 PDS を直接参照 (stg entry 自体を plan に置かない)
- **`vconn` → augment**: `materialization: live_pds` で [tableau-pds-augmenter](../.claude/skills/tableau-pds-augmenter/SKILL.md) が Live PDS として publish (run フェーズなし)
...
**目安**: intermediate は **原則 1 entity 1 .tfl**（30+ ノードまで許容）。連鎖分割は例外扱い — 詳細は [intermediate-decomposition.md](../.claude/skills/tableau-prep-architect/references/intermediate-decomposition.md) を参照。
...
連鎖分割パターン・分割の目安・actions 単位分割の判断基準は [intermediate-decomposition.md](../.claude/skills/tableau-prep-architect/references/intermediate-decomposition.md) を参照。本ファイルは「どのレイヤに属するか」、intermediate-decomposition.md は「intermediate 内をどう分けるか」の責務分担。
...
これらは decompose の出力（[decomposition-plan-format.md](decomposition-plan-format.md) の `Actions-level splits` セクション）に **ユーザーが確認できる粒度で書き出す**。実装は tableau-prep-builder が設計案に厳密に従って行う。
...
- intermediate 分解戦略: [../.claude/skills/tableau-prep-architect/references/intermediate-decomposition.md](../.claude/skills/tableau-prep-architect/references/intermediate-decomposition.md)
