...
ユーザーは **target のフルパス** を指定する（例 `"99_Sandbox/Q4-2026/flow241407_decompose"`）。上位の中間階層も target 自身も、未作成なら preflight が承認付きで埋める。target 直下の `flows/` / `datasources/` と各々の下の dbt 3 レイヤは preflight が無条件作成 (session intake の target 指定が合意を兼ねる)。
