...
prep-deployer ─ publish + run   レイヤ単位 (stg → int → marts) で publish → run → finishCode=0 確認
                                同レイヤ内は並列可、レイヤ間は順次
        ↓
prep-output-comparator ─ compare  元と分解後の最終 PDS を Metadata API + Tableau MCP で比較 → Markdown + JSON
                                  (原因分析・修正提案は持たない)
        ↓
...
| [prep-output-comparator](.claude/skills/prep-output-comparator/SKILL.md) | 元フロー最終 PDS と分解後フロー最終 PDS を Metadata API + Tableau MCP で比較し、機械的差分を Markdown + JSON で出力（原因分析・修正提案は持たない、`context: fork`） | ローカル（ファイル生成）、Cloud は **読み取りのみ** |
...
| サブフォルダ | 入れるもの | 入れないもの |
|---|---|---|
| `reports/` | prep-extractor の `flow-summary.md` / `deploy-context.md`、prep-architect の `analysis-*.md` / `decomposition-plan-*.md`、prep-output-comparator の `comparison-report.md` / `.json` / `pairs.json` | スクリプト、.tfl |
