...
7 ステップ (入力展開 → 構造把握 → トポロジ復元 → actions inventory → Mermaid DAG → Warnings 集約 → 書き出し) で構成。詳細手順・エラーハンドリング・サーバー DL の補助コマンド・制約は [references/flow-extraction-procedure.md](references/flow-extraction-procedure.md) を Read で取得。
