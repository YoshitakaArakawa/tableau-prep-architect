...
引き渡し表は [SKILL.md §後段への引き渡し](../SKILL.md#後段への引き渡し) を正典とする。補足: prep-builder は `input-dispatch-mech.json` を直接参照する必要はない (architect が decomposition-plan に embed する) が、`Materialization=live_pds` 宣言の最終検証 (= 該当 Input が本当に vconn か) のみ flow_io で再確認する (silent fallback 禁止、[../../prep-builder/references/build-recipe.md](../../prep-builder/references/build-recipe.md) 参照)。
