---
purpose: prep-extractor Phase B (cloud structure + input dispatch) の詳細手順。target_path walk による Cloud project hierarchy 取得と、flow.json + deploy-context.md からの Input 分類 + PDS LUID 解決を 1 フェーズ内で完結させる
...
Phase B は **Cloud project hierarchy の取得** と **Prep flow の Input ノード分類 + PDS LUID 解決** を 1 フェーズで担う。出力は 2 ファイル: `deploy-context.md` (Cloud structure) + `input-dispatch-mech.json` (Input dispatch、mechanical findings)。**ユーザー確認は持たない** (policy 提案 / rename 翻訳 / provisioning 確認は architect Stop 2 に統合)。

SKILL.md からは 1 行サマリでしか参照されないため、本ファイルに詳細を集約する。
...
詳細は [prep-architect/references/review-checkpoints.md](../../prep-architect/references/review-checkpoints.md) の Tier 1。
...
- **prep-architect (analyze / decompose)**: `flow-summary.md` (Phase A) + `deploy-context.md` + `input-dispatch-mech.json` (Phase B) を入力に取る
- **prep-builder**: `input-dispatch-mech.json` を直接参照する必要はない (architect が decomposition-plan に embed する)。ただし `Materialization=live_pds` 宣言の最終検証 (= 該当 Input が本当に vconn か) のみ flow_io で再確認する (silent fallback 禁止、build-recipe.md 参照)
- **prep-deployer (preflight)**: `deploy-context.md` の `pending_segments` を作成
