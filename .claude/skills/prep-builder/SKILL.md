...
3. 各 plan entry を **stg dispatch** で振り分け:
   - `Materialization: live_pds` (stg のみ) → 元 flow.json の対応 Input ノードを `inspect_input_node()` で確認 (vconn 必須、それ以外は build 中断 + escalation)。`vconn_input_to_augmenter_columns()` で列メタを取り出し、plan の Transforms 表と合わせて [prep-pds-augmenter](../prep-pds-augmenter/SKILL.md) spec を組み立て、`flows/staging/<name>.augmenter.json` に書き出す (.tfl は作らない)
...
5. 生成サマリをユーザーに報告 (kind=tfl と kind=pds_augment の件数を別途レポート)
