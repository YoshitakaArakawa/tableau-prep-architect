...
3. 各 plan entry を stg dispatch で振り分け:
   - Materialization=live_pds (stg のみ) → augmenter spec を emit
     ...
     c. vconn_input_to_augmenter_columns() で列メタを取得
     d. plan の Transforms (column-level) 表と組合せて augmenter spec を JSON 出力
     e. flows/staging/<name>.augmenter.json に書き出す
   - それ以外 (Materialization=tfl + int + marts) → .tfl を組む:
     ...
     g. zip 化して .tfl として保存
   ↓
4. 生成サマリをユーザーに報告 (kind=tfl と kind=pds_augment の件数を別途)
```
...
- passthrough PDS: input-dispatch.md の confirm 時点で既存。build 時に `deploy-context.md` から LUID を引いて実在検証可能
