...
builder は両方を **同じ LoadSqlProxy ノード** で表現する (Step 3b の `add_pds_input` + `wire_new_input_to_child` パターンと同一)。違いは `project_name` の与え方だけ — Case 1 は `ctx["layer_projects"]["stg"]["ds_project"]["name"]`、Case 2 は plan の Project path の **leaf 名**。
