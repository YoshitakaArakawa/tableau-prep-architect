---
purpose: prep-builder が新規 .tfl ファイル群を生成するための具体手順 (core)
note: 設計案パース → 元 .tfl 展開 → ノード抽出・接続再構成 → zip 化 → verify → manifest init の全 build 共通手順を規定。条件付き分岐は live-pds-augmenter-recipe.md (Materialization=live_pds) と special-outputs-recipe.md (Rename-back / incremental) に分離 — plan に該当 entry があるときだけ読む
---
...
- `--output` 既存ファイルなしの場合: `publish-manifest.json` を `status="pending"` / `luid=null` で新規生成 (decomposition-plan の Output mapping 表 + flow-summary の元 output PDS + `flows/` スキャン結果をマージ)
