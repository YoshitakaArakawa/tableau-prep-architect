...
値比較・parity 判定は持たず、事後 parity は [prep-output-comparator](../prep-output-comparator/SKILL.md) へ橋渡しする。
...
- **下流 mart / cross-flow consumer の再 run** — [prep-deployer](../prep-deployer/SKILL.md) の `run_flow.py` (FULL) に委譲 (Step 7)。**受け入れ incremental 1 サイクル (Step 6) は本 Skill が担う** — prep-deployer の `run_flow.py --incremental` を直接呼んで実走し、二重化ゼロ・正しい追記を確認する (backfill 継続性の実証なので backfill 工程の一部)
