...
本 Skill は `context: fork` を **付けない**。本番 Overwrite の失敗観測と、ユーザー判断 2 ゲート (seam/replace 選択・本番承認) を主会話で扱うため (prep-deployer と同じ整理)。

役割対称性: **書き込み = prep-deployer + prep-pds-augmenter + prep-pds-backfiller / 読み取り = prep-extractor + prep-output-comparator**。値比較・parity 判定は持たず、事後 parity は comparator へ橋渡しする。
...
- **受け入れ incremental / 下流 mart の run** — [prep-deployer](../prep-deployer/SKILL.md) の `run_flow.py --incremental` に委譲
- **値同値・parity 判定** — [prep-output-comparator](../prep-output-comparator/SKILL.md) に委譲 (挿入行は逐語コピーなので値同値は構造保証、行数 parity のみ確認すれば足りる)
...
- [ ] Step 7: 下流再 run (prep-deployer に委譲)
- [ ] Step 8: schedule 再開 (UI)

失敗時は `backfill_pds.py --spec <spec> --restore <tag>` で snapshot から即時ロールバック。
...
`.env` の `SERVER` / `SITE_NAME` を読み、OAuth 2.0 (Authorization Code + PKCE) のブラウザサインイン。Repo 直下 [scripts/tableau_auth.py](../../../scripts/tableau_auth.py) の `signed_in_server()` を import。詳細は [prep-deployer/references/authentication.md](../prep-deployer/references/authentication.md)。
