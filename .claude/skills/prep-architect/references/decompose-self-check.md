...
`decomposition-plan-<flow>.md` を出力する直前に **必ず通す** 17 項目チェックの正典。
...
各 .tfl ごとに `Upstream lineage` 表を埋める ([decomposition-plan-format.md](../../../../references/decomposition-plan-format.md) の Lineage closure invariant 節)。各 Included step が「どの宣言 Input から Prev チェーンで到達できるか」を 1 行で書く。
...
`## Output mapping (original → decomposed)` セクションが埋まっているか。元フローの全 output PDS と、それを引き継ぐ marts レイヤ flow の対応が表で書かれているか ([decomposition-plan-format.md](../../../../references/decomposition-plan-format.md) の Output mapping 節)。本表が欠けると prep-builder の `publish_manifest.py init` が失敗し、prep-output-comparator がペアを組めない。
