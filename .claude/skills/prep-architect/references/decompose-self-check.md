...
`decomposition-plan-<flow>.md` を出力する直前に **必ず通す** 13 項目チェック。SKILL.md からは 1 行サマリでしか参照されないため、各項目の **判定基準と典型 anti-pattern** を本ファイルに集約する。
...
1〜12 で不整合や検討漏れが見つかったら、是正してから plan を出力する。
