...
| 4 | **rename 翻訳** (非 ASCII caption の semantic translation: `数量` → `quantity` 等) | 列単位 | 行ごと翻訳上書き | 業務文脈依存。翻訳が外れると BI 側参照名が誤訳のまま固まる |
...
| 6 | **Output mapping** (元 output PDS → 分解後 output PDS の対応) | output 単位 | 統合 / 分割の指示 | parity 比較 (prep-output-comparator) の土台。誤ると比較対象がズレる |
...
[decompose-self-check.md](decompose-self-check.md) の 14 項目はここに入る。ユーザーに見せず Agent 内部で潰す。machine-verifiable で、ユーザーが判断する材料を持たない (元 .tfl の Prev チェーンを記憶していない、SuperUnion の暗黙列を知らない等)。
