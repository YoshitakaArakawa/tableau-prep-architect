...
`decomposition-plan-<flow>.md` を生成した後、prep-builder に渡す前に **1 回だけユーザー確認 (Stop 2)** を取る。本ファイルはその確認で何を見せ何を省くかの観点を 3 Tier で定義する。
...
| 4 | **rename 翻訳** (非 ASCII caption の semantic translation: `数量` → `quantity` 等) | 列単位 | 行ごと翻訳上書き | 業務文脈依存。stg/int の内部命名として全下流 .tfl の式に焼き込まれるため後から変えると広範囲 rebuild |
| 5 | **整備依頼リスト** (direct_db / extract Input → vconn 化 / PDS 化案) | Input 単位 | 「整備します」「partial build で進める」を選択 | session 進行判断。整備未完で build に進むと当該 stg は skip される |
| 6 | **Output mapping + Rename-back** (元 output PDS → 分解後 output PDS の対応、および mart 境界の列レベル rename-back 表) | output 単位 + 列単位 | 統合 / 分割の指示、rename-back 行の上書き | parity 比較 (prep-output-comparator) の土台。mart 出力列名は元 output PDS と完全一致が規範 ([../../../references/decomposition-plan-format.md §Rename-back](../../../references/decomposition-plan-format.md)) — 既存 Workbook の field 束縛が名前ベースのため、不一致は repoint 時の invalid field に直結 |
...
応答後、main agent は `decomposition-plan-<flow>.md` を修正反映して書き直し、prep-builder を起動する (確認の二重ループは作らない)。修正が広範に及ぶ場合は本 Skill を `mode=decompose` で再起動する。
...
[decompose-self-check.md](decompose-self-check.md) の 15 項目はここに入る。ユーザーに見せず Agent 内部で潰す。machine-verifiable で、ユーザーが判断する材料を持たない (元 .tfl の Prev チェーンを記憶していない、SuperUnion の暗黙列を知らない等)。

主な項目 (詳細は self-check 参照):

- Upstream lineage 表の埋め (Prev 連鎖が Inputs に到達)
- SuperUnion の削除候補化禁止 (`Table Names` 暗黙列)
- 列削除 actions の元順序保全
- 分岐ノード下流の列要件 divergence → intermediate 分割
- Joins cardinality 明示
- Dependency DAG (Before/After 2 ブロック)
- 同一ソース Input の staging 集約
- marts AddCol / Filter の上流降ろし検討
- 型変換 / 名前変換の staging 集中
- `Materialization=live_pds` の stg Transforms 表に `rename` / `cast` / `hide` 以外の op が混じっていない
- mart Rename-back 表のカバレッジ (内部名の露出ゼロ、表の中身自体は Tier 1 #6 で確認)

prep-builder の `verify_lineage_closure` / `verify_edge_namespaces` で二重防御も効いているため、decompose 段階で潰しきれなくても build で機械的に弾かれる (ただし decompose で潰すほうがやり直しが安い)。
