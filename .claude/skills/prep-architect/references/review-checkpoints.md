...
> **Stop 1 との境界**: 本ファイルが定義するのは Stop 2 (フロー単位の分解設計確認)。プロジェクト単位の割り付け (scope / 移行順 / 横断工程適用 / 人間作業段取り) は Stop 1 で、[prep-migration-planner](../../prep-migration-planner/references/orchestration-model.md) が扱う。
