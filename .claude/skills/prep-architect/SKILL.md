...
| `input_dispatch_path` | decompose で **必須** (status=confirmed のもの) | prep-extractor Phase C の `input-dispatch.md`。各 Input の取扱 (passthrough / augment / block) と Transforms 提案を読む。block 行があれば decompose を中断 |
...
analyze の結果と `input-dispatch.md` (confirmed) を基に、新 .tfl の構成・命名・配置・migration order を設計した `decomposition-plan-<flow-name>.md` を出す。

| 項目 | 内容 |
|---|---|
| 入力 | `flow-summary.md`, `analysis-<flow-name>.md`, `input-dispatch.md` (status=confirmed) |
| 出力 | `decomposition-plan-<flow-name>.md`（書式: [../../../references/decomposition-plan-format.md](../../../references/decomposition-plan-format.md)）|
| 主な判断 | レイヤ分割、actions レベル分割、サブプロジェクト配置、移行順序、**stg materialization** (input-dispatch から導出) |
...
**input-dispatch の各 Input 行から plan を組み立てるルール**:

| dispatch 値 | plan 内の扱い |
|---|---|
| `passthrough` | stg entry を **作らない**。元 PDS を消費する下流 entry の `Inputs` に直書き (project_path + name + LUID を input-dispatch から転記) |
| `augment` | stg entry を作り `Materialization: live_pds`、input-dispatch の Transforms 提案を `Transforms (column-level)` 表として転記 (列 UUID + caption + datatype を input-dispatch から、to_caption / to_datatype は提案値) |
| `tfl` (将来的に non-vconn な複雑 stg が必要な場合) | stg entry を作り `Materialization: tfl`、従来の Included original steps / Upstream lineage を記述 |
| `block` | **decompose を中断**。block 理由をメイン会話に返してユーザーに Cloud 側整備を依頼。plan ファイルは出力しない |
...
`decomposition-plan-<flow>.md` を出力する前に **必ず 13 項目チェック** を通す。詳細な判定基準と典型 anti-pattern は [references/decompose-self-check.md](references/decompose-self-check.md) を Read で取得。要約:
