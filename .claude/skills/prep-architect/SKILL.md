...
| `output_dir` | ✅ | `analysis-<flow>.md` / `decomposition-plan-<flow>.md` の出力先 (典型: `work/<yyyymmdd>_<tag>/`) |
...
4. 1〜3 で不整合が見つかったら、ステップ配置・削除提案を是正してから plan を出力する
...
| 長いフロー (30+ ステップ) | `work/<date>/analysis-<flow>.md` / `work/<date>/decomposition-plan-<flow>.md` ファイル + 会話にサマリ |
