...
analyze の結果と `input-dispatch-mech.json` を基に、分解設計を **`decomposition-plan-<flow>.json`** (機械可読、[../../../references/plan-json-schema.md](../../../references/plan-json-schema.md)) として出す。ユーザーレビュー用の `decomposition-plan-<flow>.md` は **スクリプトが plan.json からレンダリングする** — 手書きしない。
...
| `decompose` | `<output_dir>/decomposition-plan-<flow>.json` + `.md` (render_plan_md.py 産物) |
| `both` | 上記両方 |
