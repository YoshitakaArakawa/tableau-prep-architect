...
`decomposition-plan-<flow>.md` を出力する前に **必ず 17 項目チェック** を通す。詳細な判定基準と典型 anti-pattern は [references/decompose-self-check.md](references/decompose-self-check.md) を Read で取得。要約:
...
15. pds 入力の出所を分類 (in-scope フロー出力 = 暫定 passthrough + 差し替え追跡メモ / 外部 raw = 恒久)、vconn 入力は既存 stg との同一テーブル一致で再利用をデフォルト検討
16. incremental input / append 出力があれば継承方針・履歴 backfill 要否・期間一致での parity 検証方法を plan に明示 (Stop 2 Tier 1)
17. 1〜16 で不整合があれば是正してから出力
