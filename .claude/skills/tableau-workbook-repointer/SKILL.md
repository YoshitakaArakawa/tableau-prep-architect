...
**repoint-design.json** と **repoint-runbook.md** を 1 パスで生成する (両者は必ず一致)。
join モデル (luid 主キー / name fallback `match: "name"` / `unmapped_old_pds`) の正典は
[publish-manifest-format.md §repoint join model](../../../references/publish-manifest-format.md)、
WB 側の読み方は [lineage-model.md](references/lineage-model.md)。
...
repoint の事前条件として tableau-pds-comparator で検証済みが前提 ([twb-surgery.md](references/twb-surgery.md))。
