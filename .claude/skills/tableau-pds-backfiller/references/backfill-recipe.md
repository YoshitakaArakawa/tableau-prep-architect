---
purpose: tableau-pds-backfiller のライフサイクル (0-9) の詳細手順。各ステップの目的・スクリプト・ゲート・委譲先を規定する
note: SKILL.md の checklist の実行本体。スクリプトは skill の scripts/ にあり、cross-skill の run / 再 run は tableau-prep-deployer に委譲する
---
...
7 下流再 run       ──(tableau-prep-deployer に委譲)
8 schedule 再開    ──(UI)
...
```bash
python ${CLAUDE_SKILL_DIR}/scripts/check_flow_readiness.py --flow-luid <accumulator-flow-luid>   # --window-minutes 60 (既定)
```
...
```bash
python ${CLAUDE_SKILL_DIR}/scripts/resolve_accumulator.py --flow-luid <flow-luid> --out accumulators.json
```
...
```bash
python ${CLAUDE_SKILL_DIR}/scripts/diff_pds_schema.py old.metadata.json new.metadata.json --control <control> --json-out diff.json
```
...
```bash
python ${CLAUDE_SKILL_DIR}/scripts/backfill_pds.py --spec backfill-spec.json --only <tag>
```
...
```bash
python ${CLAUDE_SKILL_DIR}/scripts/publish_preview.py --spec backfill-spec.json --workdir <workdir> \
...
```bash
python ${CLAUDE_SKILL_DIR}/scripts/backfill_pds.py --spec backfill-spec.json --only <tag> --execute
```
...
続けて **受け入れ incremental を 1 サイクル**実走し、二重化ゼロ・正しい追記を確認する (継続性の実証)。これは backfill 工程の一部なので**本 Skill が** tableau-prep-deployer の `run_flow.py` を直接呼ぶ (他 skill のスクリプトなので repo root からの相対パス):

```bash
python .claude/skills/tableau-prep-deployer/scripts/run_flow.py --flow-id <accumulator-flow-luid> --incremental
```
...
- run 自体は tableau-prep-deployer の `run_flow.py` に委譲 (書き込み責務の分離)。本 Skill は下流の特定と run-type (FULL) の指定まで
...
```bash
python ${CLAUDE_SKILL_DIR}/scripts/backfill_pds.py --spec backfill-spec.json --restore <tag>
```
