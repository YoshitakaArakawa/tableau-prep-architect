...
役割対称性: 読み取り = prep-extractor + prep-output-comparator + **prep-schedule-designer** / 書き込み = prep-deployer (+ augmenter)。
...
| `trigger_policy` | design | ✅ | ドメイン別トリガの合意事項を文章で。既定は「元スケジュール踏襲 + 毎日」。曜日限定 (例: 市場データ系は平日) は**ユーザーが指定したドメインのみ**適用 (Skill は業務特性を推定しない)。元スケジュールの実測は Step 2 の probe が出す |
...
メイン会話への戻り値の末尾に **`## Timing` ブロック** を必ず含める ([skill-timing-contract.md](../../../references/skill-timing-contract.md))。verify モードの戻り値には overall_verdict と要対応事項の要約も含める。
...
既存 runFlow タスク・Linked Task (メンバー順含む)・**対象フローへのスケジュール衝突**・**stale look-alike フロー**を読み取る。認証失効時は `signed_in_server()` がブラウザサインインを促すため、失敗したら caller にその旨を返す (fork 内で放置してタイムアウトさせない)。

### Step 3: ドメイン設計

- schedule-inputs.json の **components (連結成分) を最小単位**とし、`trigger_policy` と probe の旧スケジュール実測 (どのフロー群が同一トリガだったか) を踏まえて 1 Linked Task = 1 ドメインに束ねる。相互依存の無い成分の束ねはトリガ 1 本化の運用判断 — 元スケジュールの単位を既定とする
...
スクリプトが失敗したら**その時点で停止し、caller にエラーを返す** (autonomous-recovery はしない。読み取り専用なのでリトライ判断は caller)。よくある失敗:
...
Python: `tableauserverclient` (repo の requirements.txt に含まれる)。repo 共通の `scripts/tableau_auth.py` / `scripts/flow_io.py` を import する。
