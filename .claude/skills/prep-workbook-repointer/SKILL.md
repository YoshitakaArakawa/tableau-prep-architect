...
publish-manifest の突合で **機械確定** し、(a) 人間が Desktop の Replace Data Source で再現できる
runbook にする、または (b) repoint モードが TWB 手術で自動実行する。いずれの経路でも差し替え後は
同じ lineage を再走査して verify する。
...
runbook のパス・対象 WB 数・ペア数・`unmapped_old_pds` 件数・warnings を要約して返す。差し替えは
(a) repoint モード (rehearsal から) か (b) 人間の Desktop 作業のどちらでも進められる旨を添える。
