...
計画書に載る情報は 3 分類。**流用してよいのは決定だけ**。これは `tableau-schedule-designer/references/scheduling-model.md` の「run-type を設計文書から転記するな」を計画書全体へ一般化したもの。
...
3. **同じ値を `tableau-schedule-designer` の既存 `trigger_policy` 引数に渡す** (Skill は計画書の存在を知らない)
...
- **接続の書き換え・スケジュール作成はしない**: それぞれ `tableau-workbook-repointer` (repoint モードの自動手術が既定、fallback は人間の Replace Data Source) / `tableau-schedule-designer` (人間の UI Linked Task) の領分。planner は段取りを台帳化するだけ。
