...
[Phase 4]  prep-workbook-repointer     design: 旧 PDS 参照 WB の棚卸し + 旧→新 PDS 対応
                                       ...
                                       → 差し替えは 2 経路のどちらか:
                                         (a) repoint モード: rehearsal 手術 publish → 証拠比較を
                                             ユーザーに提示 → 明示承認 → production Overwrite (サーバー書込)
                                         (b) 人間が Desktop の Replace Data Source で差し替え + republish
...
| **Q2b. 横断工程** | schedule / repoint / backfill の複数選択 (省略可 = なし) | 任意 | migration-planner の `--crosscut` にそのまま渡る。schedule はトリガ方針 (曜日限定ドメインの有無) の確認が追加で要る ([prep-schedule-designer](../prep-schedule-designer/SKILL.md))。repoint は移行完了済み・旧 WB が Cloud 上に存在することが前提で、schedule とは独立 (片方だけ可) ([prep-workbook-repointer](../prep-workbook-repointer/SKILL.md))。backfill は計画への事前登載のみで、実行は compare 後の明示要求 + 段取りゲート ([prep-pds-backfiller](../prep-pds-backfiller/SKILL.md)) |
