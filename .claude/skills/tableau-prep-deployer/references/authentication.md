...
`tableau-prep-deployer` / `tableau-prep-extractor` / `tableau-pds-augmenter` の認証方針と `.env` 運用ルール。
...
`context: fork` で起動する Skill (tableau-prep-builder / tableau-prep-deployer / tableau-prep-extractor Phase B) は別プロセスだが、`.auth-cache/session.json` を共有するため **最初の 1 fork で OAuth が走ればそれ以降の fork はキャッシュ hit でブラウザ起動なし**。並列 fork が同時に miss を踏んだ場合は両方が再 OAuth を発火しうるが、後勝ちで cache が上書きされるだけで害はない。
