---
purpose: tableau-pds-augmenter の spec.json 入力フォーマット仕様。source 3 種 (extract / live / vconn) ごとの必須/任意フィールド、transforms / calcs / 出力ファイル定義
...
caller (= 通常 tableau-prep-builder) が vconn テーブルの列を 1 つずつ enumerate する。flow.json の Input ノードに列メタが揃っている前提。
