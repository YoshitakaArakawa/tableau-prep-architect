---
purpose: tableau-prep-extractor Phase A (flow extraction) の 4 ステップ実装手順
...
```bash
python .claude/skills/tableau-prep-extractor/scripts/gen_flow_summary.py \
    work/<date>/flow.json \
...
- 次に推奨するアクション（例: 「tableau-prep-architect の analyze フェーズへ」）
...
python .claude/skills/tableau-prep-extractor/scripts/list_flows.py --name-contains "<flow name>"

# DL
python .claude/skills/tableau-prep-extractor/scripts/download_flow.py \
    --flow-id <LUID> \
...
- **業務的解釈・レイヤ推定はしない**（それは tableau-prep-architect analyze の役割）
