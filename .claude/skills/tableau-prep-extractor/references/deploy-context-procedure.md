---
purpose: tableau-prep-extractor Phase B (server structure + input dispatch) の詳細手順。target_path walk による Tableau Server/Cloud project hierarchy 取得と、flow.json + deploy-context.md からの Input 分類 + PDS LUID 解決を 1 フェーズ内で完結させる
...
Phase B は Tableau Server/Cloud の **project hierarchy の取得** と **Prep flow の Input ノード分類 + PDS LUID 解決** を 1 フェーズで担う。出力は 2 ファイル: `deploy-context.md` (server structure) + `input-dispatch-mech.json` (Input dispatch、mechanical findings)。**ユーザー確認は持たない** (policy 提案 / stg rename / provisioning 確認は architect Stop 2 に統合)。
...
`get_project_structure.py` は path を walk し、**存在する prefix（`existing_chain`）** と **作成すべき残り（`pending_segments`）** に分割する。後段の tableau-prep-deployer preflight はそれをループで埋める。
...
8. **Next step** — tableau-prep-architect / tableau-prep-deployer への引き渡し
...
```bash
python .claude/skills/tableau-prep-extractor/scripts/get_project_structure.py \
...
```bash
python .claude/skills/tableau-prep-extractor/scripts/dispatch_inputs.py \
...
6. `## Datasources in scope` (project_path / name / luid) は **常に書き出す** — scan 範囲は target subtree、`--also-scan` で渡した親プロジェクトの subtree を union する。scope 内に PDS が無ければ `_(none ...)_` 行になる (target が pending で also-scan も無ければ空)
...
| 項目 | architect での処理場所 |
|---|---|
| stg rename の元名ピン留め (正規化 rename の反映を含む) | decompose の Transforms 表 ([input-policy.md §命名レジーム](../../../../references/input-policy.md)) |
...
- 読み取り専用 — サブプロジェクト作成や権限変更は **tableau-prep-deployer の preflight** に委譲
...
引き渡し表は [SKILL.md §後段への引き渡し](../SKILL.md#後段への引き渡し) を正典とする。補足: tableau-prep-builder は `input-dispatch-mech.json` を直接参照する必要はない (architect が decomposition-plan に embed する) が、`Materialization=live_pds` 宣言の最終検証 (= 該当 Input が本当に vconn か) のみ flow_io で再確認する (silent fallback 禁止、[../../tableau-prep-builder/references/build-recipe.md](../../tableau-prep-builder/references/build-recipe.md) 参照)。
