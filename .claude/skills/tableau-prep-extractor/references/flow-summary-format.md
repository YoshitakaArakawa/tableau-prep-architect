---
purpose: tableau-prep-extractor が flow.json から生成する flow-summary.md の書式仕様
note: トップレベル構造、Meta / Topology / Dependency DAG / SuperTransform actions inventory / Warnings の各セクションの厳密書式を規定
---

# flow-summary-format

**`tableau-prep-extractor` Skill** の出力（`flow-summary.md`）の書式仕様。本 Skill が flow.json を読んでこの形式に変換する。後段 (`tableau-prep-architect` の analyze/decompose/build) は **flow.json を直接読まず、`flow-summary.md` のみ** を参照する。
