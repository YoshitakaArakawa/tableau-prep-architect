---
purpose: tableau-prep-extractor Phase B が出力する input-dispatch-mech.json の JSON スキーマ仕様。architect が consume する mechanical findings のフォーマット
...
`work/<session>/reports/input-dispatch-mech.json` の JSON スキーマ。Phase B の `dispatch_inputs.py` が生成、tableau-prep-architect が読む。**ユーザー確認のための markdown ではない** — 単なる mechanical findings の永続化。
...
`augmenter_columns_hint` は tableau-pds-augmenter の vconn-source 入力フォーマットに整形済の列メタ。builder が `Materialization=live_pds` の stg を build するときにそのまま augmenter spec の `columns` フィールドに渡せる。
