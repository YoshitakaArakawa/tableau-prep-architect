...
- 後段 Skill（tableau-prep-architect）は **flow-summary.md / deploy-context.md / input-dispatch-mech.json のみを入力** とすればよく、責務分離が明確になる
...
- tableau-prep-deployer の preflight が「サブプロジェクト不足 → 作成承認を取る」判断材料を必要とする
...
| 後段 Skill | 渡すファイル |
|---|---|
| tableau-prep-architect (analyze / decompose) | `flow-summary.md` + `deploy-context.md` + `input-dispatch-mech.json` (+ 複数フロー移行時は `flow-dependencies.md`) |
| tableau-prep-builder | `decomposition-plan-<flow>.json`（tableau-prep-architect 出力、設計の正） |
| tableau-prep-deployer (preflight) | `deploy-context.md` |
| tableau-prep-deployer (publish) | `flows/**/*.tfl` + `flows/staging/*.augmenter.json` + `deploy-context.md` |
