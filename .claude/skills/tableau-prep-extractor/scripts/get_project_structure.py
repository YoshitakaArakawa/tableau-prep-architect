...
`deploy-context.md` describing:
  ...
  - Missing parents/layers (consumed by tableau-prep-deployer preflight)
...
def resolve_path(projects, path: str):
    ...
    segment is created by tableau-prep-deployer's preflight with a separate user approval.
...
def render(target, target_path, target_status, target_writeable,
    ...
    tableau-prep-deployer's preflight creates everything top-down.
    """
    ...
    lines.append("generated_by: tableau-prep-extractor/get_project_structure.py")
    ...
    lines.append("Consumed by tableau-prep-architect (decompose) and tableau-prep-deployer (preflight + publish).")
    lines.append("")
    ...
    lines.append("Trailing segments may be `pending`; tableau-prep-deployer preflight creates them one ")
    ...
    if target is not None:
        ...
        lines.append(f"- Status: **pending** — tableau-prep-deployer preflight will request approval to create the missing segments.")
    lines.append("")
    ...
    if target is None:
        ...
        if missing_parents:
            lines.append(f"**Missing parents**: {', '.join(f'`{n}`' for n in missing_parents)}. "
                         "tableau-prep-deployer preflight will create these under target.")
        if missing_layers:
    ...
    # subtrees. Consumed by tableau-prep-extractor Phase B (dispatch_inputs.py) to
    ...
    lines.append("")
    lines.append("- **tableau-prep-architect (decompose)** — uses `## Existing flows` to avoid name collisions.")
    lines.append("- **tableau-prep-deployer (preflight)** — iterates `pending_segments` to reach target, ")
    ...
    lines.append("- **tableau-prep-extractor Phase B (dispatch_inputs.py)** — reads `## Datasources in scope` to ")
