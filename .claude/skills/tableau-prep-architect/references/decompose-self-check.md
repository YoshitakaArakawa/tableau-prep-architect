...
tableau-prep-builder の build 開始時にも `verify_lineage_closure` / `verify_edge_namespaces` で機械的に二重防御するが、decompose 段で潰しておくほうがやり直しが安い (build → publish → run fail → rebuild の loop が 10-15 min)。

**操作のシリアライズ形式は本チェックに影響しない**: flow-summary.md の actions inventory は flat SuperTransform / Container (` [container 形式]`) / Input renames (` [Input renames]`) の 3 形式を同じ形で収録する ([flow-summary-format.md](../../tableau-prep-extractor/references/flow-summary-format.md#supertransform-actions-inventory))。以下の全項目は操作の **type と topological 位置** で判定するので、どの形式でも扱いは同一。難読化フロー (列名が UUID) では表示名 (日本語含む) が Input renames と各 field caption に現れる — stg 境界の rename ピン留めや列削除順序 (項目 11) を読むときはこの層を参照する。build 時は `normalize_source_containers` が Container を flat 化するので actions 分割も形式非依存。
...
元フローの全 output PDS それぞれに、それを引き継ぐ flow の `source_original_output_name` が plan.json 上で割り当てられているか (存在しない output 名は構造検証が弾くが、**割り当て漏れ = null のまま** は機械検証されない)。漏れると tableau-pds-comparator がペアを組めない ([decomposition-plan-format.md](../../../../references/decomposition-plan-format.md) の Output mapping 節)。
...
## 14. 命名レジーム順守と Rename-back (通常は空)

命名レジーム ([input-policy.md §命名レジーム](../../../../references/input-policy.md)) の下では、stg の `to_caption` が元の内部名にピン留めされている限り **mart の列名 parity は自動達成され、`rename_back` は空が正常**。確認すること:

1. stg transforms の `to_caption` に semantic translation (英語化等) が混入していないか — 原則は現行 caption そのまま、例外は actions-split で stg に吸収した正規化 rename のみ。ズレは render の式トークン照合でも機械検知されるが、上流に kind=tfl が混ざるフローは照合 skip されるため目視も残す
2. `rename_back` を書くのは **上流で divergent な forward rename を意図的に導入した例外ケースのみ** (原理と書式は [decomposition-plan-format.md §Rename-back](../../../../references/decomposition-plan-format.md))。その場合はサフィックス派生列 (`_買付` 等) を含む全カバーを照合する
...
- **in-scope フローの出力** → passthrough は**暫定**。plan に「上流 (フロー名) の移行完了後に新 PDS へ差し替え」の追跡メモを Output mapping 近くに残す。移行順は producer 先行が原則 (依存の実抽出は tableau-prep-extractor Phase C の `flow-dependencies.md`。無ければ deploy-context の出所プロジェクトと PDS 命名から判断し、`未確認` とラベルする)
...
同一 vconn テーブルから stg が 2 本できるのは、命名がピン留めと分裂する二重管理の始まり。既存 stg の公開列名 (元名ピン留め済み) をそのまま前提にする。
...
- **履歴 backfill は分解のスコープ外 (別 Skill が担う)**: 分解時の既定は旧 output PDS をアーカイブ残置し継承層は baseline 以降を前方累積。履歴が硬要件なら、移行後の別工程 [tableau-pds-backfiller](../../tableau-pds-backfiller/SKILL.md) が旧 PDS から seed する (分解の自動一気通貫には含めず、ユーザー明示要求 + 段取りゲート付き)。plan には baseline-forward の帰結 (下流の履歴依存計算が当面劣化しうる) を明記する
