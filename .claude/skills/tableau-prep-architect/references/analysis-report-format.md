---
purpose: tableau-prep-architect の analyze フェーズが出力する分析レポートの markdown 書式仕様
...
`flow-summary.md` は別 Skill `tableau-prep-extractor` が生成する（[../../tableau-prep-extractor/SKILL.md](../../tableau-prep-extractor/SKILL.md)）。analyze 開始時に `flow-summary.md` が無い場合は、まず `tableau-prep-extractor` を起動するよう案内する。
