---
purpose: tableau-prep-architect decompose 完了後のユーザー確認 (Stop 2) における観点を 3 Tier で規定する。Tier 1 は明示確認必須、Tier 2 はデフォルト受諾、Tier 3 は Agent 自律で Stop 2 に出さない
...
`decomposition-plan-<flow>.md` + `.html` を生成した後、tableau-prep-builder に渡す前に **1 回だけユーザー確認 (Stop 2)** を取る。提示は **`.html` のパスを案内してブラウザで開いてもらう** のを主とする (As-is → 分解先マップと依存 DAG が視覚確認できる。md は git 追跡の設計記録 + ターミナル fallback)。本ファイルはその確認で何を見せ何を省くかの観点を 3 Tier で定義する。
...
> **Stop 1 との境界**: 本ファイルが定義するのは Stop 2 (フロー単位の分解設計確認)。プロジェクト単位の割り付け (scope / 移行順 / 横断工程適用 / 人間作業段取り) は Stop 1 で、[tableau-prep-migration-planner](../../tableau-prep-migration-planner/references/orchestration-model.md) が扱う。

## Tier 1 — 明示確認必須

ユーザーから `OK` または `<行番号> <修正指示>` の応答が来るまで tableau-prep-builder に進まない。
...
| 5 | **Output mapping** (元 output PDS → 分解後 output PDS の対応) | output 単位 | 統合 / 分割の指示 | parity 比較 (tableau-pds-comparator) の土台。mart 出力列名は元 output PDS と完全一致が規範だが、命名レジーム (元名 end-to-end 保持、[../../../../references/input-policy.md §命名レジーム](../../../../references/input-policy.md)) の下では自動達成される — 確認対象は対応関係のみ |
...
応答後、main agent は `decomposition-plan-<flow>.json` の該当フィールドを修正し `render_plan_md.py` で md + html を再レンダリングして、tableau-prep-builder を起動する (md / html を直接編集しない。確認の二重ループは作らない)。修正が広範に及ぶ場合は本 Skill を `mode=decompose` で再起動する。
...
tableau-prep-builder の `verify_lineage_closure` / `verify_edge_namespaces` で二重防御も効いているため、decompose 段階で潰しきれなくても build で機械的に弾かれる。
