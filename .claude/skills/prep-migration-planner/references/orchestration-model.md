...
- **接続の書き換え・スケジュール作成はしない**: それぞれ `prep-workbook-repointer` (repoint モードの自動手術、または人間の Replace Data Source) / `prep-schedule-designer` (人間の UI Linked Task) の領分。planner は段取りを台帳化するだけ。
