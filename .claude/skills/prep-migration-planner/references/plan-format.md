...
- **backfill_candidates**: 各 in-scope フローの incremental 判定 (`run_type == "incremental"`) から機械抽出。`control_field` は `control_fields[0]`。`mode` は init では常に `null` (compare 後にユーザーが seam/replace を決める)。
